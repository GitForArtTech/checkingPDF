...
async function extractFormFieldsFromPdfAndExportToExcel() {
  try {
    // 讀取本地端的 PDF 檔案
    // const filePathFrom = process.cwd() + "/ActivityHall.pdf";
    const filePathFrom = "./outputPDF/ActivityHall.pdf";
    ...
    // 申請日期
    let applyDate = "";
    const fillFormYearField = form.getTextField("fillFormYear").getText();
    const fillFormMonthField = form.getTextField("fillFormMonth").getText();
    const fillFormDayField = form.getTextField("fillFormDay").getText();
    if (
      fillFormYearField != undefined &&
      fillFormMonthField != undefined &&
      fillFormDayField != undefined
    ) {
      applyDate = `${fillFormYearField}年 ${fillFormMonthField}月 ${fillFormDayField}日`;
    } else {
      applyDate = "無填寫";
    }
    ...
    // 要檢查的 Excel 檔案路徑
    const filePath = process.cwd() + "/ActivityHall.xlsx";
    // const filePath = "ActivityHall.xlsx";
